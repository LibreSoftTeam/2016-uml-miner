#!/usr/bin/python3
"""
uml_file, file_id, repo_id, repo_name, author_id, author, commiter, modifications, repo_age, file_age, commits_before, commits_after
...
with open('uml_files.csv', 'wt') as csvfile:
    ...
    for values in files:
        ...
        b = query('author, author_id, committer_id, date', 'scmlog, actions', 'type="A" AND actions.commit_id=scmlog.id AND file_id=' + str(values['id']))
        print(b)
        if not b:
            continue
        author = b['author']
        ...
        writer.writerow([uml_file, file_path, file_url, file_id, repo_id, repo_user, repo_name, author_id, author, committer, modifications, repo_age, file_age, commits_before, commits_after])
